...
/**
However, what happens if ``e2`` contains free variables? The danger here is that they may be accidentially "captured" by the substitution. For instance, consider
...
Hence we are still not done with defining substitution. But what is the desired result of the substitution above? The answer is that we must avoid the name clash by renaming the variable
 bound by the "lambda" if the variable name occurs free in ``e2``. This new variable name should be "fresh", i.e., not occur free in ``e2``.
